...
const data_dragon_1 = require("data-dragon");
const LOL = __importStar(require("./riotapi/api"));
...
let games = [];
const LoadWatchList = () => {
    if (!fs_1.default.existsSync("./data/watchlist.txt")) {
        fs_1.default.mkdirSync("./data");
        fs_1.default.writeFileSync("./data/watchlist.txt", "", { flag: "w" });
        return;
    }
    watchlist.length = 0;
...
client.on("ready", async () => {
    ...
    dragon = await data_dragon_1.DataDragon.latest();
    ...
    if (command === "losestreak") {
        ...
        try {
            ...
            let messageEmbed = new discord_js_1.EmbedBuilder()
                .setTitle(summoner.name)
                .setColor(0x00FF00)
                .setFields({ name: "Time Elapsed", value: `${minutes}:${seconds}` });
            for (let participant of gameInfo.participants) {
                ...
                if (teams[participant.teamId] == null)
            ...
            Object.entries(teams).forEach(([key, value]) => {
                messageEmbed.addFields({ name: `Team ${i}`, value: `${value}`, inline: true });
                ++i;
            });
            /*for (let participant of gameInfo.participants) {
                const participantChamp = dragon.champions.find((champion) => champion.key === String(participant.championId))
            ...
            messageEmbed.addFields({ name: `Banned`, value: `${bans}`, inline: false });
            await interaction.editReply({ embeds: [messageEmbed] });
        }
        catch (err) {
            if (err.response) {
        ...
        const messageEmbed = new discord_js_1.EmbedBuilder()
            ...
            .setColor(0x00fdfd)
            .setFields({ name: "Name", value: randGame.name }, { name: "Link", value: randGame.link })
